...
!!!!!!!!!!!!!!
       if(i.le.nt_opcyc) then
           do j=1,npos
            ...
            ctemp(j)= dconjg(chi1(j))*xmu_chi1(j)+dconjg(chi2(j))*xmu_chi2(j) 
...
!********************************************************************
domega= 2.d0*pi/(nt_hhg*delt)
open(unit=1010,name="HHG.dat",status='replace') 
call fourier(proj_xmu_chi_big,nt_hhg,1,delt,domega)
!! ATTENTION: Il faut replacer les elements du vecteur-resultat de cette FT
HHG_simple_v=cdabs(proj_xmu_chi_big)**2
do j = 1, npos	
     call fourier(xmu_chi1_big(:,j),nt_hhg,1,delt,domega)	
