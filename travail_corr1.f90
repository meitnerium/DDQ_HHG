...
double precision :: xmin, xmax, requ, diss,massreduite, morse,lieprobv,tmpreal,domega
...
double precision :: alpha,delr,p0,rdeb,proj(npos) ,xmue,auto_correl(ntps) 
...
! 
double complex, allocatable ::xmu_chi1(:,:),xmu_chi2(:,:),proj_HHG(:,:),proj_xmu_chi(:)    ! pour calcul du spectre HHG
!
!***********************************************************************
...
enddo
do i=1,1024
	   call simpson(npos,delr,dreal(proj_HHG(i,:)),tmpreal)  !!! ATTENTION: Il faut replacer les elements du vecteur-resultat des FT s
           HHG(i) = dcmplx(tmpreal,0.d0)
enddo
 
!	    	Écriture  du spectre `HHG` dans un fichier: A TRAVAILLER.
